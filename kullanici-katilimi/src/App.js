...
let formSchema = Yup.object().shape({
  name: Yup.string()
    .min(5, "İsim soyisim en az 5 karakter olmalı")
    .required("İsim soyisim doldurulmalıdır."),
...
function App() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      ...
      .then((response) => {
        setRegisterMember(response.data);
        setForm(dummyForm); // Formu başarılı bir şekilde gönderdikten sonra temizle
      })
